...
def plan_pick_and_place(req):
    ...
    hand_close_trajectory = plan_hand_trajectory(hand_move_group, [0.0, 0.0])
    ...
    response.trajectories.append(hand_close_trajectory)
    response.trajectories.append(pick_up_pose)
    response.trajectories.append(pre_place_pose)
    response.trajectories.append(place_pose)
    response.trajectories.append(hand_open_trajectory)

    # It is advised to clear the targets after planning the poses ---------------------------------
...
def plan_trajectory(move_group, destination_pose, start_joint_angles):
    ...
    # If planning fails, raise an exception
    if not plan:
        exception_str = """
