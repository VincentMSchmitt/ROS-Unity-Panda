...
import control.positions as robot_values
...
def opening_Hand(services:dict, width:float):
    ...
    # Set the Opening Width requested
...
def closing_Hand(services:dict, width:float):
    ...
    # Set the requested procedure of Hand-Service to 'close' (id = 2)
    node = client.get_node(services['Hand-Service']['procedure_control']['ProcedureOp']['NodeID'])
    node.set_value(2)
    ...
    # Set the Closing Width requested
...
def moving_robot_via_joints(services:dict, joint_values:list):
    # Set the Move-Service to Operator Mode
    node = client.get_node(services['Move-Service']['op_src_mode']['StateOpOp']['NodeID'])
    node.set_value(True)
    
    # Set the requested procedure of Move-Service to move via joint values (id = 1)
    node = client.get_node(services['Move-Service']['procedure_control']['ProcedureOp']['NodeID'])
    node.set_value(1)

    # Set the Procedure Parameters to Operator Mode
...
def moving_robot_via_posquat(services:dict, pos:list, quat:list):
    ...
    # Set the requested procedure of Move-Service to move via posquat values (id = 2)
    node = client.get_node(services['Move-Service']['procedure_control']['ProcedureOp']['NodeID'])
    node.set_value(2)
